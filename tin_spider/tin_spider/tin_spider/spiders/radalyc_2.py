...
def escribir_csv(data, filename='articulos_redalyc.csv'):
    with open(filename, 'w', newline='', encoding='utf-8') as f:
        writer = csv.DictWriter(f, fieldnames=[
            "titulo", "año", "autores", "revista", "palabras", "resumen", "link"])
        writer.writeheader()
        for item in data:
            writer.writerow(item)

def main():
    datos_articulos_csv = 'articulos_redalyc.csv'
    ...
    while True:
        ...
        try:
            with open(datos_articulos_csv, 'r', newline='', encoding='utf-8') as f:
                reader = csv.DictReader(f)
                datos_existente = list(reader)
        ...
        escribir_csv(datos_existente, datos_articulos_csv)
